...
/** INTENT_SENTINEL / collectPendingIntentIds / Turn moved to
 *  components/chat/turn.tsx so the full conversation page reuses the same
 *  message renderer (inline intent cards included). */
...
export function CommandPalette() {
  ...
  useEffect(() => {
    ...
    // Programmatic open — clicking a conversation in the sidebar / list fires
    // this with { conversationId } so the palette opens directly onto that
    // thread (Claude-style: click = continue). Without an id (e.g. the
    // dashboard quick-open) it just opens onto the active conversation.
    ...
    window.addEventListener("yen:open-palette", onOpenRequest);
    return () => {
      window.removeEventListener("keydown", onKeyDown);
      window.removeEventListener("yen:open-palette", onOpenRequest);
    };
  }, [open, status, stop]);
